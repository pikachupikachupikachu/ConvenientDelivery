...
public class ToastUtil {
	...
	public static void showError(Context context, int errorCode){
		...
		try {
	        ...
	        default:
	        	Toast.makeText(context,"查询失败："+errorCode , Toast.LENGTH_LONG).show();
                logError("查询失败", errorCode);
                break;
	        }
        } catch (Exception e) {
            Toast.makeText(context, e.getMessage(), Toast.LENGTH_LONG).show();
            logError(e.getMessage(), errorCode);
        }
	}

    private static void logError(String info, int erroerrorCode) {
        print(LINE);//start
        print("                                   错误信息                                     ");
        print(LINE);//title
        print(info);
        print("错误码: " + erroerrorCode);
        print("                                                                               ");
